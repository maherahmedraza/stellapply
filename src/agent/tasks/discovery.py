...
def execute(self, task_id: str, user_id: str, payload: Dict[str, Any]):
    """
    Celery task wrapper for the Scout Agent.
    """
    task_uuid = uuid.UUID(task_id)
    user_uuid = uuid.UUID(user_id)

    logger.info(f"Starting Scout Agent task {task_id}")

    async def _run_agent():
        agent = ScoutAgent(user_uuid, task_uuid)
        return await agent.run(payload)

    try:
        # Run the async agent in the sync Celery worker
        result = async_to_sync(_run_agent)()
        logger.info(f"Scout Agent task {task_id} completed: {result}")
        return result
    except Exception as e:
        logger.error(f"Scout Agent task {task_id} failed: {e}")
